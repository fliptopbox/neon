...
// .wrangler/tmp/bundle-Eqpvy9/checked-fetch.js
...
async function authMiddleware(c, next) {
  ...
  try {
    const isValid2 = await verify(token, c.env.JWT_SECRET);
    if (!isValid2) {
...
app2.put("/:id", authMiddleware, async (c) => {
  const id = c.req.param("id");
  const data = await c.req.json();
  const [venue] = await query(
    c.env,
...
app3.put("/:id", authMiddleware, async (c) => {
  const id = c.req.param("id");
  const data = await c.req.json();
  const [host] = await query(
    c.env,
...
app4.put("/:id", authMiddleware, async (c) => {
  const id = c.req.param("id");
  const data = await c.req.json();
  const [session] = await query(
    c.env,
...
app5.get("/by-user/:userId", async (c) => {
...
app5.post("/", authMiddleware, async (c) => {
  ...
  const [model] = await query(
    ...
    [
      ...
      data.active !== void 0 ? data.active : 1
    ]
  );
  return c.json(model, 201);
});
app5.put("/:id", authMiddleware, async (c) => {
  const id = c.req.param("id");
  const data = await c.req.json();
  ...
  const [model] = await query(
    ...
    [
      ...
      data.account_sortcode,
      data.active !== void 0 ? data.active : 1,
      data.sells_online !== void 0 ? data.sells_online : 0,
      id
    ]
  );
  if (!model) {
...
app5.delete("/:id", authMiddleware, adminMiddleware, async (c) => {
  const id = c.req.param("id");
...
app6.post("/", authMiddleware, async (c) => {
  const user = c.get("user");
...
app6.put("/:id", authMiddleware, async (c) => {
  const id = c.req.param("id");
  const data = await c.req.json();
  const [artist] = await query(
    c.env,
...
app6.delete("/:id", authMiddleware, adminMiddleware, async (c) => {
  const id = c.req.param("id");
  await query(c.env, "UPDATE artists SET active = 0 WHERE id = $1", [id]);
  return c.json({ message: "Artist deleted" });
});
var artists_default = app6;
var app7 = new Hono2();
app7.get("/", authMiddleware, adminMiddleware, async (c) => {
...
app7.get("/me", authMiddleware, async (c) => {
  const user = c.get("user");
...
app7.put("/me/bio", authMiddleware, async (c) => {
  const user = c.get("user");
  const data = await c.req.json();
...
app7.put("/:id", authMiddleware, adminMiddleware, async (c) => {
  const id = c.req.param("id");
  const data = await c.req.json();
  await query(
    c.env,
...
app7.patch("/:id/toggle", authMiddleware, adminMiddleware, async (c) => {
  const id = c.req.param("id");
...
app7.patch("/:id/toggle-admin", authMiddleware, adminMiddleware, async (c) => {
  const id = c.req.param("id");
  await query(
    c.env,
...
app7.delete("/:id", authMiddleware, adminMiddleware, async (c) => {
  const id = c.req.param("id");
...
app8.post("/", authMiddleware, async (c) => {
  const user = c.get("user");
  const data = await c.req.json();
...
app8.delete("/:id", authMiddleware, async (c) => {
  const id = c.req.param("id");
  const user = c.get("user");
  await query(
    c.env,
...
var calendar_default = app10;
var app11 = new Hono2();
app11.get("/stats", async (c) => {
  try {
    const sql = Ys(c.env.DATABASE_URL);
    const venuesTotal = await sql`SELECT COUNT(*) as count FROM venues`;
    const venuesActive = await sql`SELECT COUNT(*) as count FROM venues WHERE active = 1`;
    ...
    const modelsActive = await sql`SELECT COUNT(*) as count FROM models WHERE active = 1`;
    ...
    const usersAdmin = await sql`SELECT COUNT(*) as count FROM users WHERE is_admin = true`;
...
app12.use(
  ...
  cors({
    origin: /* @__PURE__ */ __name((origin) => {
      ...
      if (origin.endsWith(".neon-admin.pages.dev") || origin === "https://neon-admin.pages.dev") {
        return origin;
      }
...
app12.route("/api/auth", auth_default);
app12.route("/api/dashboard", dashboard_default);
app12.route("/api/hosts", hosts_default);
app12.route("/api/venues", venues_default);
app12.route("/api/sessions", sessions_default);
app12.route("/api/venue-tags", venue_tags_default);
app12.route("/api/calendar", calendar_default);
app12.route("/api/models", models_default);
app12.route("/api/artists", artists_default);
app12.route("/api/users", users_default);
app12.route("/api/images", images_default);
...
// .wrangler/tmp/bundle-Eqpvy9/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-Eqpvy9/middleware-loader.entry.ts
