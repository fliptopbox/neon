...
// .wrangler/tmp/bundle-kpFRxB/checked-fetch.js
...
app3.get("/by-user/:userId", async (c) => {
...
var artists_default = app4;
var app5 = new Hono2();
app5.get("/", authMiddleware, adminMiddleware, async (c) => {
  const users = await query(
    c.env,
    `SELECT u.id, u.emailaddress, u.active, u.created_on, u.login_on, u.is_admin,
            ub.fullname, ub.known_as, ub.description
     FROM users u
...
app5.get("/me", authMiddleware, async (c) => {
  const user = c.get("user");
...
app5.put("/me/bio", authMiddleware, async (c) => {
  const user = c.get("user");
  const data = await c.req.json();
...
app5.patch("/:id/toggle", authMiddleware, adminMiddleware, async (c) => {
  const id = c.req.param("id");
...
app5.patch("/:id/toggle-admin", authMiddleware, adminMiddleware, async (c) => {
  const id = c.req.param("id");
  await query(
    c.env,
...
app6.delete("/:id", authMiddleware, async (c) => {
  const id = c.req.param("id");
  const user = c.get("user");
  await query(
    c.env,
...
var calendar_default = app8;
var app9 = new Hono2();
app9.get("/stats", async (c) => {
  try {
    const sql = Ys(c.env.DATABASE_URL);
...
app10.route("/api/dashboard", dashboard_default);
app10.route("/api/venues", venues_default);
app10.route("/api/venue-tags", venue_tags_default);
app10.route("/api/calendar", calendar_default);
app10.route("/api/models", models_default);
app10.route("/api/artists", artists_default);
app10.route("/api/users", users_default);
app10.route("/api/images", images_default);
app10.notFound((c) => c.json({ error: "Not found" }, 404));
app10.onError((err, c) => {
  console.error(`${err}`);
  return c.json({ error: "Internal server error" }, 500);
});
var api_default = app10;
...
// .wrangler/tmp/bundle-kpFRxB/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-kpFRxB/middleware-loader.entry.ts
