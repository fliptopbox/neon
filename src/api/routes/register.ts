...
app.post("/model", async (c) => {
  ...
  try {
    ...
    // Truncate description to 1024 chars to prevent abuse
    if (
      data.description &&
    ...
    const [newUser] = await query<User>(
      c.env,
      `INSERT INTO users (email, password_hash, is_global_active, is_admin, tz, date_created) 
       VALUES ($1, $2, true, false, $3, NOW()) 
       RETURNING id`,
      [data.email, hashedPassword, data.tz || "Europe/London"]
    );
    ...
    if (
      data.portrait_urls &&
      Array.isArray(data.portrait_urls) &&
      data.portrait_urls.length > 0
    ) {
    ...
    if (newUserId) {
      try {
        ...
        console.error(
          `Cleanup failed for user ${newUserId}: ${cleanupErr.message}`
        );
      }
    }
